...
if ($_POST['set-gsm-port']){
	$port = $_POST['gsm-port'];
	if ($port == 'ttyUSB0' || $port == 'ttyUSB1' || $port == 'ttyUSB2' || $port == 'ttyUSB3' || $port == 'ttyUSB4' ||
...
if ($_POST['set-modem-device']){
	...
	if ($port == 'ttyUSB0' || $port == 'ttyUSB1' || $port == 'ttyUSB2' || $port == 'ttyUSB3' || $port == 'ttyUSB4' ||
		$port == 'usb0' || $port == 'usb1' || $port == 'usb2' || $port == 'usb3' || $port == 'usb4') {
		exec("sudo uci set network.3g.device=/dev/".$port);
		exec('sudo uci commit network');
		$status = "Set modem device untuk interface 3g sukses ($port)";
	}
	else {
		$status = "Invalid Port";
	}
...
if ($_POST['set-iface']) {
	...
	if ($interface == 'eth2' || $interface == 'eth1' || $interface == '3g' || $interface == 'wan1' || $interface == 'usb0') {
		$file = file_get_contents('/home/ab/gsm');
		$pattern = "/IFACE '.*'/";
		...
		file_put_contents('/home/ab/gsm', $file);
		$interface = str_replace('eth2', 'E3372 HiLink (Gl.iNet)', $interface);
		$interface = str_replace('eth1', 'E3372 HiLink (HG553/OrangePiZero)', $interface);
		$interface = str_replace('usb0', 'MF831 HiLink', $interface);
		$interface = str_replace('wan1', 'LTE', $interface);
...
if ($show == "home") {
	...
	$iface = array("3g"=>"3g", "E3372 HiLink (GL.iNet)"=>"eth2", "E3372 HiLink (HG553/OrangePiZero)"=>"eth1", "MF831 HiLink"=>"usb0");
	?>
	<br>
	<form action="" method="post">
		Modem Interface :
